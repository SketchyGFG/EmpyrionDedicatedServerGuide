...
Instructions regarding an Empyrion Server:
...
1 - INITIAL INSTALLATION
	...
	-Disable 8dot3 naming to the target drive: https://learn.microsoft.com/en-us/windows-server/administration/windows-commands/fsutil-8dot3name
...
1.3 - Install PeaZip
	...
	Steps:
	1.) Download PeaZip for windows from the following page: https://peazip.github.io/peazip-64bit.html
...
1.4 - Install OSFMount
	...
	Steps:
	1.) Download OSFMount from the following page: https://www.osforensics.com/tools/mount-disk-images.html
...
1.5 - Place ESM files
	...
	Steps:
	1.) Download the latest ESM software from the following location: https://github.com/Vollinger0/esm/releases
...
1.6 - Install base game
	...
	2.) Use the following command to install the base game: d:\steamcmd\steamcmd.exe +force_install_dir d:\empyrion\ +login anonymous +app_update 530870 validate +quit
	3.) Use the following command *again* to validate the base game: d:\steamcmd\steamcmd.exe +force_install_dir d:\empyrion\ +login anonymous +app_update 530870 validate +quit


1.7 - Install required C++ code libraries

	With the game installed there should now be a folder located at: D:\empyrion\_CommonRedist\vcredist\2019.  In this folder there should be the files VC_redist.x64.exe and VC_redist.x86.exe.  These installers need to be executed (just double click them) before the base game will run.  It to be possible to install more recent versions of these executables as well.
...
1.8 - Install NSSM
	...
	Steps:
	1.) Download the NSSM software from the following page: https://nssm.cc/download.  Using the pre-release version is preferred.
...
1.9.1 - ESM
	...
	Configuration and files to be found in the location d:\esm
...
1.9.2 - EAH
	...
	EAH configuration and files to be found in the location d:\empyrion\DedicatedServer\EmpyrionAdminHelper\Config
	Do not rename these files expect to make backups.
	Setup of the EAH is best done from within the EAH where settings are consolidated into interface.
...
1.9.4 - NSSM
	...
	Commands:
	d:\nssm\win64\nssm.exe install Empyrion_StarterService
		or
	...
	Commands:
	d:\nssm\win64\nssm.exe install Empyrion_SharedDataServer
		or
...
1.11 - Backups
	...
	3.) Add the following line to the EAH timetables, the EAH will replace the timestamp when processed: <TT Ld="2024-07-23T03:17:41.5493315-04:00" W="13" A="17" P="" P2="" DZ="" CC="DFA531" AC="true" />
...
1.12 - Ramdisk first time setup
	...
	Steps:
	1.) From an administrative command line execute the following command: d:\esm\esm.exe check-requirements
		Review feedback and follow any guidance.
	2.) From an administrative command line execute the following command: d:\esm\esm.exe ramdisk-install
		This process may start the game momentarily to create save game files, then reorganize them for migration to the ramdisk.
	3.) From an administrative command line execute the following command: d:\esm\esm.exe ramdisk-setup
		This process creates the ramdisk and migrates the save game to it.
	4.) From an administrative command line execute the following command: d:\esm\esm.exe check-requirements
		Review feedback and follow any guidance.
...
1.13 - Initial game startup
	...
	2.) Start the EAH locally from path: D:\empyrion\DedicatedServer\EmpyrionAdminHelper\EmpyrionAdminHelper.exe
	...
	5.) Look for the 'Open File - Security Warning' dialogs to appear and un-check the option 'Always ask before opening this file' for each one.  There should be 3 in all.  If this something about this does not look right, stop the server and try again from step 3.
...
2.1 - Using SteamCMD to update game
	...
	Steps:
	1.) Exit the game.
	2.) From an administrative command line execute the following command: d:\steamcmd\steamcmd.exe +force_install_dir d:\empyrion\ +login anonymous +app_update 530870 validate +quit
...
4.3 - Stopping the EAH
	...
	2.) Watch the process list to ensure that both the EmpAdminHelper_NoGUI.exe and EmpAdminHelper_NoGUI.exe exit.  You may need to exit the process if that doesn't happen.
...
4.8 - Erase existing game to start new game
	...
	Steps:
	1.) From an administrative command line execute the following command: d:\esm\esm delete-all --doit
		This will remove the ramdisk and delete any data from the previous save.  This can fail at times.
	2.) Use the EAH to apply desired game settings.
	3.) From an administrative command line execute the following command: d:\esm\esm.exe ramdisk-install
		This process may start the game momentarily to create save game files, then reorganize them for migration to the ramdisk.  Also, you may have to manually help the game exit for the install process to continue.  Look for a button on the game server screen to exit the game.  The rest will usually take care of itself.
	4.) From an administrative command line execute the following command: d:\esm\esm.exe ramdisk-setup
		This process creates the ramdisk and migrates the save game to it.
	5.) From an administrative command line execute the following command: d:\esm\esm.exe check-requirements
		Review feedback and follow any guidance.
...
8 - Empyrion Web Access(EWA)
	...
	Steps:
	1.) Download the latest EWA software archive from the following location: https://github.com/GitHub-TC/EmpyrionWebAccess/releases
...
10 - Telnet
	...
	Telnet is important for admin tools to function. For this guide the telnet server resides on port 30004.
